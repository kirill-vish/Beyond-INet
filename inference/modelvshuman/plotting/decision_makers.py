...
class DecisionMaker:
    ...
    def __post_init__(self):
        ...
        assert len(
            self.df.subj.unique()) > 0, "no 'subj' column found in data frame"

        if self.plotting_name is None:
            if type(self.name_pattern
                    ) is str and self.name_pattern in self.df.subj.unique():
        ...
        if len(self.decision_makers) == 0:
            print("The following model(s) were not found: " +
                  ', '.join(self.name_pattern))
        ...
        assert self.marker in Line2D.markers.keys(), "Unknown marker " + str(
            self.marker
        ) + " used when creating decision maker: use one of " + str(
            Line2D.markers.keys())
...
def decision_maker_to_attributes(decision_maker_name, decision_maker_list):
    ...
    raise ValueError("No attributes found for decision maker " +
                     decision_maker_name)
