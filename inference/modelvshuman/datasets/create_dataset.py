...
def resize_crop_image(input_file, resize_size, crop_size):
...
def create_dataset(original_dataset_path, target_dataset_path, rng,
                   min_num_imgs_per_class, max_num_imgs_per_class,
                   target_resize_size, target_crop_size):
    ...
    for c in sorted(os.listdir(original_dataset_path)):
        ...
        if human_category is not None:
            class_count_dict[human_category] += len(
                os.listdir(pjoin(original_dataset_path, c)))
            for image_name in sorted(
                    os.listdir(pjoin(original_dataset_path, c))):
                image_path_dict[human_category].append(
                    pjoin(original_dataset_path, c, image_name))
    ...
    for human_category in sorted(hc.get_human_object_recognition_categories()):
        ...
        choice = rng.choice(num_images,
                            num_imgs_per_target_class,
                            replace=False)
        ...
        for image_index in choice:
            ...
            target_image_path = pjoin(
                target_dataset_path, human_category,
                human_category + "-" + image_index_str + "-" +
                image_path.split("/")[-1].replace("_", "-"))
...
def create_experiment(expt_name,
    ...
    assert (os.path.exists(expt_source_dir)
            ), "directory " + expt_source_dir + " does not exist."
    ...
    for i in range(0, num_subjects + 1):
        ...
        target_dir = pjoin(expt_target_dir, expt_name, subject_name,
                           "session-1")

        if os.path.exists(target_dir):
            print("Error: target directory " + target_dir +
                  " does already exist.")
            sys.exit(1)
        else:
        ...
        for i, img_index in enumerate(order):
            ...
            target_image_path = pjoin(
                target_dir,
                (get_leading_zeros(i + 1) + "_" + expt_abbreviation + "_" +
                 subject_abbreviation + "_" + condition + "_" +
                 correct_category + "_" + "00_" + imgname))
